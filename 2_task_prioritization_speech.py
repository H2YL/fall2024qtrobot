...
# Set up QTrobot speech services
say_service = rospy.ServiceProxy('/qt_robot/speech/say', speech_say)
recognize_service = rospy.ServiceProxy('/qt_robot/speech/recognize', speech_recognize)
...
def get_speech_input():
    say_service("Please speak after the beep.")
    say_service('#CAR HORN#')
    resp = recognize_service("en_US", [], 10)  # No specific words to recognize, 10-second timeout
...
def main():
    ...
    while True:
        ...
        if "1." in response and "- " in response:
            ...
            say_service("Your tasks have been prioritized. Say 'exit' to end the conversation and save your prioritized task list.")

    if tasks:
        save_tasks_to_csv(tasks)
        say_service("Your prioritized task list has been saved.")
